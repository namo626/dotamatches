...
team :: String -> Scraper T.Text T.Text
team side = chroot ("span" @: ["class" @= ("opp " ++ side)]) $ text "span"

-- | Parses the left team's name
leftTeam = team "opp1"
-- | Parses the right team's name
rightTeam = team "opp2"
...
data MatchException = MatchException    -- ^ The scraper didn't find anything on the detail link of a match
  deriving Show

instance Exception MatchException where
  displayException MatchException = "No data found"
...
processTour p url = do
  ...
  case tourPage of
    Nothing -> throwIO MatchException
    Just tn -> return tn
